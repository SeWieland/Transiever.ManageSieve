...
The protocol reader handles fragmented input, quoted strings, byte-counted literals,
capability data, and `OK`, `NO`, and `BYE` completion responses.
...
Script payloads are bytes rather than strings because literal sizes are byte counts and downloaded content must be preservable exactly.
